...
public class HarpoonRope : MonoBehaviour
{
    ...
    void DrawRope()
    {
        if (!strightLine)
        {
            if (m_lineRenderer.GetPosition(percision - 1).x == HarpoonGun.grapplePoint.x)
            {
                strightLine = true;
            }
            else
    ...
    void DrawRopeWaves()
    {
        for (int i = 0; i < percision; i++)
        {
            ...
            Vector2 offset = Vector2.Perpendicular(HarpoonGun.grappleDistanceVector).normalized * ropeAnimationCurve.Evaluate(delta) * waveSize;
